...
The hypothesis, which is preliminary and untested, is that anomaly detection may be better suited to markets than traditional alpha models because it does not require a stable predictive relationship between features and returns. It only requires that the model of "normal" be accurate enough that deviations are detectable, and that some fraction of those deviations precede significant price moves. Whether this is true, and whether it can be made to work after transaction costs, is the subject of future posts.
